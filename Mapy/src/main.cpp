...
void performTreeTest(int n, std::vector<int>& keys)
{
...
void performHashTest(int n,std::vector<int>& keys) {
...
int main(int argc, char** argv)
{
  ...
  */
  srand(time(nullptr));
  auto numberOfRepeat = argc > 1 ? std::atoll(argv[1]) : 10000;

  std::vector<int> keys;
  for(auto i = 0; i < numberOfRepeat; ++i)
    keys.push_back(i);
